...
/**
 * @description: 修改后台用户和权限关系表 除角色中定义的权限以外的加减资源，加权限是指用户比角色多出的权限，减权限是指用户比角色少的权限处理器
 * @author: created by hang.yu on 2020-01-13 23:04:04
 */
@Component
public class UpdateUserPermissionHandler extends AbstractCommonHandler<UpdateUserPermissionRequestDTO, UserPermissionDO, Long> {

    @Autowired
    private UserPermissionMapper userPermissionMapper;

    @Override
    public BaseResult<Long> validate(UpdateUserPermissionRequestDTO requestDTO) {
        return ResultUtil.success();
    }

    @Override
    public BaseResult<Long> processor(UpdateUserPermissionRequestDTO requestDTO) {
        return ResultUtil.success();
    }
