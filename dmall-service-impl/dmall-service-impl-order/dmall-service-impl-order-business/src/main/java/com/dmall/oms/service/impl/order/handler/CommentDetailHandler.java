...
public class CommentDetailHandler extends AbstractCommonHandler<Long, SubOrderDO, List<CommentDetailResponseDTO>> {
    ...
    public BaseResult<List<CommentDetailResponseDTO>> processor(Long subOrderId) {
        ...
        if (subOrderDO.getOrderItemId() != null) {
            List<CommentDetailResponseDTO> collect = data.stream().map(commentResponseDTO -> {
                CommentDetailResponseDTO responseDTO = new CommentDetailResponseDTO();
                responseDTO.setSkuId(commentResponseDTO.getSkuId());
                OrderItemDO orderItemDO = orderItemSupport.findByOrderIdAndSkuId(subOrderDO.getOrderId(), responseDTO.getSkuId());
